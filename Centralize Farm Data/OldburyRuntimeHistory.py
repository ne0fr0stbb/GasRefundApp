...
access_db_path = r"C:\Users\Jason\Desktop\Farm Ventra Databases\Oldbury11525.accdb"
...
try:
    for row in zone_info:
        for data in server_data:
            if row.DayTime == data.DayTime and row.TheHour == data.TheHour and data.Farm == "Oldbury":
                break
        else:
            # Record does not exist, insert it
            sql_server_cursor.execute(insert_query, "Oldbury", row.ControllerID, row.ZoneNum, row.DayTime, row.TheDate,
